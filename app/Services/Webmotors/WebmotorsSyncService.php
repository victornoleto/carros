...
class WebmotorsSyncService
{
    ...
    public function getPageResult(string $brand, string $model, int $page = 1): array {
        ...
        list($url, $query) = $this->getUrlAndQuery($brand, $model, $page);
    ...
    public function getAdResults(array $pageResult): array
    {
        $results = $pageResult['NewSearchResults'];

        return $results;
    }

    public function getAdData(string $brand, string $model, array $result): array
    {
        $service = new WebmotorsProcessService($brand, $model, $result);
    ...
    private function getUrlAndQuery(string $brand, string $model, int $page = 1): array
    {
        $url = '/api/search/car';

        $queryUrl = $this->serverUrl.'/carros';
        
        if (env('ONLY_GOIAS') == 1) {
            $queryUrl .= '/go';
        }

        $queryUrl .= "/$brand/$model";
