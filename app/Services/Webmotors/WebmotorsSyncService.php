...
class WebmotorsSyncService extends CarSyncService
{
    ...
    public function getPageResult(string $brand, string $model, int $page = 1, string|null $state = null): array
    {
        list($url, $options) = $this->getUrlAndOptions($brand, $model, $page, $state);
    ...
    private function getUrlAndOptions(string $brand, string $model, int $page = 1, string|null $state = null): array
    {
        $url = '/api/search/car';

        $state = $state ?? 'estoque';

        $queryUrl = $this->serverUrl."/carros/$state/$brand/$model";

        $options = [
            'headers' => self::getHeaders()
        ];

        $url = $url.'?url='.$queryUrl.'&actualPage='.$page;
